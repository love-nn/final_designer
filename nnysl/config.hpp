#ifndef __NNYSL_CONFIG_H__
#define __NNYSL_CONFIG_H__
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name
        ...
        if ( it != m_datas.end() ) {
            ...
            if ( tmp ) {
                ...
                NNYSL_LOG_ERROR(NNYSL_LOG_ROOT()) << "Lookup name = " << name << "exists but type not " << typeid(T).name() 
        ...
        if ( name.find_first_not_of("abcdefghijklmnopqrstuvwxyz._0123456789")
            != std::string::npos ) {
            NNYSL_LOG_ERROR(NNYSL_LOG_ROOT()) << "Lookup name invalid " << name ;
    ...
    static void LoadFromYaml(const YAML::Node& root) ;  // 加载yaml 配置
    static ConfigVarBase::ptr LookupBase (const std::string& name ) ;   // 查找Base 
